...
def get_linear_burst_size(t, aa):
    tlim = 15.0
    if t < tlim:
        return 1.0
    if t >= tlim:
...
#folder = 'Nu_exploring_CollectiveTimeInterval'
...
if burst_histogram == True:
    hist = [hist[i] * get_linear_burst_size(bins[i], aa=aa) for i in range(0, len(hist))]
...
if burst_histogram == False:
    ...
    ax_hist.set_title(r'MOSI=' + str(mosi) + ',' + 'k=' + str(k) + ',' + r'${n}_{d}=$' + str(nd) +
                      ',' + r'$\Delta {N}_{p}=$' + str(burst_size) + ',' + 'a=' + str(aa), fontsize=20)
...
if save_plot == True:
    ...
    if burst_histogram == True:
        ...
        figure_name = 'Phage_sigmoidal_linear' + file_name + '_' + str(p) + '_' + str(c) + '_' + str(t0) + '_' + str(
            a) + '.png'
