...
@numba.njit
def system_evolution_LO(delta_t, Nb0, B, P, N, k, nd, nu, n0, doubling_time, twindow, Nimax, Nimax_std, Nt_LOR, niter, time_window_track=True, constant_burst = False, ctburstsize=50.0):
    ...
    # Control of lysis from without resistance
    LOR = np.zeros (Nb0, dtype=np.int64)
    ...
    while n<niter:
        ...
        for ii in infected_indices:
            ...
            if r < prob_new_timer:
                ...
                # We check if the cell has developed LOR
                if LOR[ii] == 0:
                    if ninfect[ii] == 1:
                        if Nt[ii] >= Nt_LOR:
                            LOR[ii] = 1
                ...
                if LOR[ii] == 0:
                    Ntimax = Nimax
                    Ntimax_std = Nimax_std
                elif LOR[ii] == 1:
                    Ntimax = Nimax*2.0
                    Ntimax_std = Nimax_std*2.0
                
                if  ninfect[ii]>= np.random.uniform(low=Ntimax-(Ntimax_std/2.0),high=Ntimax+(Ntimax_std/2.0)):
                    tau_mins = taul_to_minutes(t,N,k) - first_infection_time[ii]
                    states[ii] = 2
                    lysis_times[ii] = tau_mins
                    burst_md_counter += 1
...
if __name__ == '__main__':
    ...
    time_vector, nphages, nhbacteria, nibacteria, ndbacteria, nnutrients, avg_ninf, avg_burst, burstmd, burstsizemd, firstinf, lysist, ninfect = system_evolution_LO(delta_t, Nb0, B, P, N, k, nd, nu, n0, doubling_time, time_window, Nimax, Nimax/4.0, 10, niter)
